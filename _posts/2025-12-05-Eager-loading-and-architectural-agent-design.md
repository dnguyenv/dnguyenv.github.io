...
A flat list of 2,000 tools is a poor design. A better approach is to use Hierarchical Toolsets.

We could, for example, have a simple "router" model (or even just a tool-use call) that first classifies the user's intent.
...
5.he Host then asks the LLM to ls the directory to "discover" the 10 files it just put there.
